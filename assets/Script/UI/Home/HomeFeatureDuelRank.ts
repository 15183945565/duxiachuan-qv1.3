...
type DuelLuanshiRankSiblingState = {
    node: Node;
    active: boolean;
};

type DuelLuanshiRankPageHost = Node & {
    duelLuanshiRankSiblingStates?: DuelLuanshiRankSiblingState[];
};
...
export abstract class HomeFeatureDuelRank extends HomeFeatureDuelRankHost {
    ...
    protected openDuelLuanshiRankPage(page: Node): void {
        const rankPage = this.ensureDuelLuanshiRankPage(page);
        this.hideDuelLuanshiPageSiblingsForRank(page, rankPage);
        rankPage.active = true;
        this.refreshDuelLuanshiRankPage(rankPage);
        this.ensureInputBlocker(rankPage);
        rankPage.setSiblingIndex((page.children.length || 1) - 1);
        this.positionDuelBackForLuanshiRank(page);
    }

    protected closeDuelLuanshiRankPage(rankPage: Node): void {
        rankPage.active = false;
        const page = rankPage.parent;
        if (page) {
            this.restoreDuelLuanshiPageSiblingsAfterRank(page);
            this.restoreDuelBackAfterLuanshiRank(page);
        }
    }

    protected hideDuelLuanshiPageSiblingsForRank(page: Node, rankPage: Node): void {
        const runtime = page as DuelLuanshiRankPageHost;
        if (!runtime.duelLuanshiRankSiblingStates) {
            runtime.duelLuanshiRankSiblingStates = page.children
                .filter((child) => child !== rankPage)
                .map((node) => ({ node, active: node.active }));
        }
        runtime.duelLuanshiRankSiblingStates.forEach((state) => {
            if (state.node?.isValid) state.node.active = false;
        });
    }

    protected restoreDuelLuanshiPageSiblingsAfterRank(page: Node): void {
        const runtime = page as DuelLuanshiRankPageHost;
        runtime.duelLuanshiRankSiblingStates?.forEach((state) => {
            if (state.node?.isValid) state.node.active = state.active;
        });
        runtime.duelLuanshiRankSiblingStates = undefined;
    }

    protected positionDuelBackForLuanshiRank(page: Node): void {
    ...
    protected restoreDuelBackAfterLuanshiRank(page: Node): void {
        const panel = page.parent;
        const back = panel?.getChildByName('DuelBack');
        if (!back) return;
        Tween.stopAllByTarget(back);
        ...
        back.active = true;
        back.setPosition(HomeConfig.DUEL_LUANSHI_BACK_X, backY, 0);
        back.setSiblingIndex((panel.children.length || 1) - 1);
    }

    protected ensureDuelLuanshiRankPage(page: Node): Node {
        const rankPage = this.getOrCreateEditorNode('LuanshiRankPage', page, HomeConfig.VIEW_WIDTH, HomeConfig.VIEW_HEIGHT, 0, 0);
        this.ensureInputBlocker(rankPage);

        const bg = this.getOrCreateEditorSkinnedNode(
            'LuanshiRankPageBackground',
            rankPage,
            HomeConfig.VIEW_WIDTH,
            HomeConfig.VIEW_HEIGHT,
            0,
        ...
        this.hideLegacyDuelLuanshiRankNodes(rankPage);
        this.ensureDuelLuanshiRankTitle(rankPage);
        this.ensureDuelLuanshiRankTopRoot(rankPage);
        this.ensureDuelLuanshiRankStreakHeader(rankPage);
        this.ensureDuelLuanshiRankStreakList(rankPage);
        this.ensureDuelLuanshiRankFooter(rankPage);
        this.refreshDuelLuanshiRankPage(rankPage);
        return rankPage;
    }

    protected hideLegacyDuelLuanshiRankNodes(rankPage: Node): void {
        [
            'LuanshiRankPageTitle',
            'LuanshiRankMetricTabs',
            'LuanshiRankListRows',
            'LuanshiRankSelfRow',
            'LuanshiRankPeriodTabs',
            'LuanshiRankPagePlaceholder',
        ].forEach((name) => {
            const node = rankPage.getChildByName(name);
            if (node) node.active = false;
        });
    ...
    protected ensureDuelLuanshiRankTitle(rankPage: Node): void {
        const titleBarExisted = !!rankPage.getChildByName('LuanshiRankStreakTitleBar');
        const titleBar = this.getOrCreateEditorSkinnedNode('LuanshiRankStreakTitleBar', rankPage, 182, 36, 0, 680, HomeConfig.UI_DUEL_JIANGHU_RANK_INVEST_TITLE_BAR);
        titleBar.active = true;
        if (!titleBarExisted) titleBar.setScale(1.5, 1.5, 1);
        titleBar.setSiblingIndex(2);

        const titleExisted = !!titleBar.getChildByName('LuanshiRankStreakTitleLabel');
        const title = this.getOrCreateDuelRoomLabel(titleBar, 'LuanshiRankStreakTitleLabel', '\u5468\u699c', 27, 0, 1.363, 130, 32, new Color(60, 44, 30, 255));
        title.string = '\u5468\u699c';
        if (!titleExisted) {
            title.fontSize = 27;
            title.lineHeight = 33;
    ...
    protected ensureDuelLuanshiRankTopRoot(rankPage: Node): Node {
        const topRootExisted = !!rankPage.getChildByName('LuanshiRankTopRoot');
        const topRoot = this.getOrCreateEditorNode('LuanshiRankTopRoot', rankPage, 700, 320, 0, 405);
        topRoot.active = true;
        if (!topRootExisted) topRoot.setScale(1.1, 1.1, 1);
        topRoot.setSiblingIndex(3);
        this.ensureDuelLuanshiRankTopCard(topRoot, 'LuanshiRankTop2', HomeConfig.UI_DUEL_JIANGHU_RANK_TOP2_BG, -233.675, -8, 198, 232, 1.1);
        this.ensureDuelLuanshiRankTopCard(topRoot, 'LuanshiRankTop1', HomeConfig.UI_DUEL_JIANGHU_RANK_TOP1_BG, 0, 24, 216, 252, 1.3);
        this.ensureDuelLuanshiRankTopCard(topRoot, 'LuanshiRankTop3', HomeConfig.UI_DUEL_JIANGHU_RANK_TOP3_BG, 234.131, -8, 198, 230, 1.1);
        return topRoot;
    }

    protected ensureDuelLuanshiRankTopCard(parent: Node, name: string, skinPath: string, x: number, y: number, width: number, height: number, scale = 1): Node {
        const cardExisted = !!parent.getChildByName(name);
        const card = this.getOrCreateEditorSkinnedNode(name, parent, width, height, x, y, skinPath);
        card.active = true;
        if (!cardExisted) {
        ...
        const metricLabel = card.getChildByName(`${name}MetricLabel`)?.getComponent(Label)
            || this.getOrCreateDuelRoomLabel(card, `${name}MetricLabel`, '', 19, 0, -86, width - 28, 34, new Color(139, 54, 20, 255));
        metricLabel.node.active = false;
        metricLabel.node.setSiblingIndex(3);
        return card;
    }

    protected ensureDuelLuanshiRankStreakHeader(rankPage: Node): void {
        const headerExisted = !!rankPage.getChildByName('LuanshiRankStreakHeader');
        const header = this.getOrCreateEditorNode('LuanshiRankStreakHeader', rankPage, 640, 42, 0, 210);
        header.active = true;
        if (!headerExisted) header.setScale(1.1, 1.1, 1);
        header.setSiblingIndex(4);
        ...
        [
            ...
            { name: 'Avatar', text: '\u5934\u50cf', x: -220, width: 70 },
            { name: 'Player', text: '\u73a9\u5bb6\u540d', x: -112, width: 160 },
            { name: 'Streak', text: '\u8fde\u80dc\u6b21\u6570', x: 82, width: 150 },
            { name: 'Reward', text: '\u5956\u52b1\uff08\u5143\u5b9d\u6570\u91cf\uff09', x: 235, width: 160 },
        ].forEach((column) => {
            const labelExisted = !!header.getChildByName(`LuanshiRankStreakHeader${column.name}Label`);
            const label = this.getOrCreateDuelRoomLabel(header, `LuanshiRankStreakHeader${column.name}Label`, column.text, 20, column.x, 0, column.width, 36, new Color(103, 64, 35, 255));
            label.string = column.text;
            if (!labelExisted) {
                this.applyDuelJianghuRankText(label, 20, new Color(103, 64, 35, 255), HorizontalTextAlignment.CENTER, 1);
            }
        });
    }

    protected ensureDuelLuanshiRankStreakList(rankPage: Node): Node {
        const listExisted = !!rankPage.getChildByName('LuanshiRankStreakList');
        const list = this.getOrCreateEditorNode('LuanshiRankStreakList', rankPage, JIANGHU_RANK_INVEST_VIEW_WIDTH, JIANGHU_RANK_INVEST_VIEW_HEIGHT, 0, -142);
        list.active = true;
        const listTransform = list.getComponent(UITransform) || list.addComponent(UITransform);
        if (!listExisted) {
            listTransform.setContentSize(JIANGHU_RANK_INVEST_VIEW_WIDTH, JIANGHU_RANK_INVEST_VIEW_HEIGHT);
        }
        list.setSiblingIndex(5);

        const listSize = listTransform.contentSize;
        const contentHeight = this.getDuelLuanshiRankStreakContentHeight();
        const content = this.getOrCreateEditorNode('LuanshiRankStreakContent', list, listSize.width, contentHeight, 0, 0);
        content.active = true;
        this.moveLegacyDuelLuanshiRankRowsIntoContent(list, content);

        const rowLayout = this.getDuelLuanshiRankStreakRowLayout(content);
        (content.getComponent(UITransform) || content.addComponent(UITransform)).setContentSize(listSize.width, rowLayout.contentHeight);
        content.setPosition(0, this.getDuelJianghuRankInvestContentTopY(list, rowLayout.contentHeight), 0);
        this.setupDuelJianghuRankInvestScroll(list, content);
        ...
        for (let index = 0; index < 10; index += 1) {
            const rank = index + 1;
            const rowY = rowLayout.firstRowY - index * rowLayout.rowStep;
            const row = this.ensureDuelLuanshiRankStreakRow(content, `LuanshiRankStreakRow_${rank}`, 0, rowY);
            row.setPosition(0, rowY, 0);
            row.setSiblingIndex(index);
        }
        return list;
    ...
    protected ensureDuelLuanshiRankStreakRow(parent: Node, name: string, x: number, y: number): Node {
        const rowExisted = !!parent.getChildByName(name);
        const row = this.getOrCreateEditorSkinnedNode(name, parent, JIANGHU_RANK_INVEST_ROW_WIDTH, JIANGHU_RANK_INVEST_ROW_HEIGHT, x, y, this.getDuelLuanshiRankStreakRowBg(name));
        row.active = true;
        if (!rowExisted) {
            (row.getComponent(UITransform) || row.addComponent(UITransform)).setContentSize(JIANGHU_RANK_INVEST_ROW_WIDTH, JIANGHU_RANK_INVEST_ROW_HEIGHT);
        }
        ...
        const rankLabel = this.getOrCreateDuelRoomLabel(row, `${name}RankLabel`, '1', 24, -307.828, 0, 60, 42, new Color(210, 102, 46, 255));
        this.applyDuelJianghuRankText(rankLabel, 24, new Color(210, 102, 46, 255), HorizontalTextAlignment.CENTER, 0);
        rankLabel.node.setSiblingIndex(0);

        this.ensureDuelJianghuRankAvatar(row, `${name}Avatar`, -242.03, 0, 54, 44);

        const nameLabel = this.getOrCreateDuelRoomLabel(row, `${name}NameLabel`, '\u5c11\u4fa0', 22, -79.535, 0, 170, 42, new Color(73, 43, 24, 255));
        this.applyDuelJianghuRankText(nameLabel, 22, new Color(73, 43, 24, 255), HorizontalTextAlignment.LEFT, 0);
        nameLabel.node.setSiblingIndex(3);

        const legacyStreakIcon = row.getChildByName(`${name}StreakYuanbaoIcon`);
        if (legacyStreakIcon) legacyStreakIcon.active = false;

        const streakLabel = this.getOrCreateDuelRoomLabel(row, `${name}StreakAmountLabel`, '0\u6b21', 22, 103.066, 9.703, 128, 42, new Color(115, 68, 29, 255));
        this.applyDuelJianghuRankText(streakLabel, 22, new Color(115, 68, 29, 255), HorizontalTextAlignment.CENTER, 0);
        ...
        const rewardLabel = this.getOrCreateDuelRoomLabel(row, `${name}RewardAmountLabel`, '0', 22, 269.066, 8.36, 104, 42, new Color(115, 68, 29, 255));
        this.applyDuelJianghuRankText(rewardLabel, 22, new Color(115, 68, 29, 255), HorizontalTextAlignment.LEFT, 0);
        rewardLabel.node.setSiblingIndex(7);
        return row;
    }

    protected getDuelLuanshiRankStreakContentHeight(): number {
        return JIANGHU_RANK_INVEST_ROW_TOP_PADDING
            + JIANGHU_RANK_INVEST_ROW_BOTTOM_PADDING
            + JIANGHU_RANK_INVEST_ROW_HEIGHT * 10
            + JIANGHU_RANK_INVEST_ROW_GAP * 9;
    ...
    protected getDuelLuanshiRankStreakRowLayout(content: Node): { contentHeight: number; firstRowY: number; rowStep: number; rowHeight: number } {
        const firstRow = content.getChildByName('LuanshiRankStreakRow_1');
        const secondRow = content.getChildByName('LuanshiRankStreakRow_2');
        const firstTransform = firstRow?.getComponent(UITransform);
        const rowHeight = firstTransform?.contentSize.height || JIANGHU_RANK_INVEST_ROW_HEIGHT;
        const rowStep = firstRow && secondRow
            ? Math.abs(firstRow.position.y - secondRow.position.y) || rowHeight + JIANGHU_RANK_INVEST_ROW_GAP
    ...
    protected moveLegacyDuelLuanshiRankRowsIntoContent(list: Node, content: Node): void {
        for (let rank = 1; rank <= 10; rank += 1) {
            const row = list.getChildByName(`LuanshiRankStreakRow_${rank}`);
            if (row) row.setParent(content);
        }
    }

    protected getDuelLuanshiRankStreakRowBg(rowName: string): string {
        const rank = Number(rowName.replace('LuanshiRankStreakRow_', ''));
        if (rank === 1) return HomeConfig.UI_DUEL_JIANGHU_RANK_ROW_TOP1_BG;
        if (rank === 2) return HomeConfig.UI_DUEL_JIANGHU_RANK_ROW_TOP2_BG;
        if (rank === 3) return HomeConfig.UI_DUEL_JIANGHU_RANK_ROW_TOP3_BG;
        return HomeConfig.UI_DUEL_JIANGHU_RANK_ROW_BG;
    ...
    protected ensureDuelLuanshiRankFooter(rankPage: Node): void {
        const footerExisted = !!rankPage.getChildByName('LuanshiRankStreakFooterLabel');
        const footer = this.getOrCreateDuelRoomLabel(rankPage, 'LuanshiRankStreakFooterLabel', '\u5b9a\u699c\u65f6\u95f4\uff1a\u4eca\u592922\uff1a00\n\u6392\u884c\u699c\u6bcf\u5929\u5237\u65b0\u4e00\u6b21', 20, 132.487, -720, 360, 66, new Color(91, 62, 38, 255));
        footer.string = '\u5b9a\u699c\u65f6\u95f4\uff1a\u4eca\u592922\uff1a00\n\u6392\u884c\u699c\u6bcf\u5929\u5237\u65b0\u4e00\u6b21';
        if (!footerExisted) {
            footer.fontSize = 20;
            footer.lineHeight = 28;
    ...
    protected refreshDuelLuanshiRankPage(rankPage: Node): void {
        if (!rankPage?.isValid) return;
        this.hideLegacyDuelLuanshiRankNodes(rankPage);

        const entries = this.getDuelLuanshiRankEntries()
            .sort((left, right) => right.streakCount - left.streakCount)
            .slice(0, 10);
        const topRoot = rankPage.getChildByName('LuanshiRankTopRoot') || this.ensureDuelLuanshiRankTopRoot(rankPage);
        [
            { nodeName: 'LuanshiRankTop1', entry: entries[0] },
            { nodeName: 'LuanshiRankTop2', entry: entries[1] },
            { nodeName: 'LuanshiRankTop3', entry: entries[2] },
        ].forEach((item) => {
            const card = topRoot.getChildByName(item.nodeName);
            if (card && item.entry) this.refreshDuelLuanshiRankTopCard(card, item.entry);
        });

        const list = rankPage.getChildByName('LuanshiRankStreakList') || this.ensureDuelLuanshiRankStreakList(rankPage);
        const content = list.getChildByName('LuanshiRankStreakContent') || list;
        entries.forEach((entry, index) => {
            const rank = index + 1;
            const row = content.getChildByName(`LuanshiRankStreakRow_${rank}`);
            if (row) this.refreshDuelLuanshiRankStreakRow(row, rank, entry);
        });
        for (let rank = entries.length + 1; rank <= 10; rank += 1) {
            const row = content.getChildByName(`LuanshiRankStreakRow_${rank}`);
            if (row) row.active = false;
    ...
    protected refreshDuelLuanshiRankTopCard(card: Node, entry: DuelLuanshiRankEntry): void {
        const name = card.getChildByName(`${card.name}NameLabel`)?.getComponent(Label);
        if (name) name.string = entry.name;
        this.applyDuelJianghuRankAvatarSkin(card, `${card.name}Avatar`, entry.gender);
        const metric = card.getChildByName(`${card.name}MetricLabel`)?.getComponent(Label);
    ...
    protected refreshDuelLuanshiRankStreakRow(row: Node, rank: number, entry: DuelLuanshiRankEntry): void {
        row.active = true;
        const rankLabel = row.getChildByName(`${row.name}RankLabel`)?.getComponent(Label);
        if (rankLabel) rankLabel.string = `${rank}`;
        const name = row.getChildByName(`${row.name}NameLabel`)?.getComponent(Label);
        if (name) name.string = entry.name;
        this.applyDuelJianghuRankAvatarSkin(row, `${row.name}Avatar`, entry.gender);
        const streakIcon = row.getChildByName(`${row.name}StreakYuanbaoIcon`);
        if (streakIcon) streakIcon.active = false;
        const streak = row.getChildByName(`${row.name}StreakAmountLabel`)?.getComponent(Label);
        if (streak) streak.string = `${entry.streakCount}\u6b21`;
        const reward = row.getChildByName(`${row.name}RewardAmountLabel`)?.getComponent(Label);
        if (reward) reward.string = this.formatDuelJianghuRankAmount(entry.rewardAmount);
    }

    protected getDuelLuanshiRankEntries(): DuelLuanshiRankEntry[] {
        return [
            { name: '\u68a6\u56de\u6c5f\u6e56', streakCount: 18, rewardAmount: 2396.8, gender: 'female' },
            { name: '\u541b\u770b\u5c71\u5dc5', streakCount: 16, rewardAmount: 2100, gender: 'male' },
            { name: '\u9ed8\u6b4c', streakCount: 15, rewardAmount: 2016, gender: 'female' },
            { name: 'nehsiac', streakCount: 13, rewardAmount: 1940.4, gender: 'male' },
            { name: '\u8f6f\u6c90\u7fa4\u4e03\u4e09', streakCount: 12, rewardAmount: 1788, gender: 'female' },
            { name: '\u725b\u9a6c', streakCount: 10, rewardAmount: 1430.6, gender: 'male' },
            { name: '\u660e\u62fe\u5c01\u4f60\u5bb6', streakCount: 9, rewardAmount: 1254.4, gender: 'female' },
            { name: '\u53f6\u843d\u77e5\u79cb', streakCount: 8, rewardAmount: 1113.2, gender: 'male' },
            { name: '\u767d\u65e0\u5c18', streakCount: 7, rewardAmount: 988, gender: 'female' },
            { name: '\u9752\u886b\u5ba2', streakCount: 6, rewardAmount: 780, gender: 'male' },
        ];
    }
}
